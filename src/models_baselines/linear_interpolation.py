"""
Linear Interpolation Baseline for Super-Resolution.

Uses scipy's RegularGridInterpolator with linear method to upsample
...
class LinearInterpolationBaseline:
    """
    Bilinear interpolation baseline for super-resolution.
    
    Interpolates low-resolution gridded data to high-resolution query positions.
    Works with both Aurora predictions and latent representations.
    """
    
    def __init__(self, method: str = "linear"):
        """
        Args:
            method: Interpolation method ('linear' for bilinear interpolation)
        """
        self.method = method
    ...
    def _interpolate_single(
        ...
        if query_lat.ndim == 1 and query_lon.ndim == 1:
            # Create meshgrid for full grid interpolation
            ...
            # Already flattened query positions
    ...
    def evaluate_on_dataset(
        ...
        for idx in tqdm(range(n_samples), desc="Linear interpolation"):
            ...
            # Compute MSE (only for matching channels)
